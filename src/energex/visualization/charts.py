...
class MarketVisualizer:
    ...
    def plot_price_quality(self, symbol: str) -> go.Figure:
        ...
        price_changes = df.with_columns([pl.col("Close").pct_change().alias("price_change")])

        fig.add_trace(
            go.Scatter(
                x=price_changes["Datetime"],
        ...
        fig.update_layout(title=f"{symbol} Price Quality Analysis", height=800, showlegend=True)
    ...
    def plot_volatility_analysis(self, symbol: str) -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                x=vol_metrics["Datetime"], y=vol_metrics["realized_vol"], name="Realized Vol"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Box(y=((df["High"] - df["Low"]) / df["Open"] * 100), name="Range Distribution"),
            row=2,
            col=1,
        )
        ...
        fig.update_layout(title=f"{symbol} Volatility Analysis", height=800, showlegend=True)
    ...
    def plot_term_structure(self, front_symbol: str, back_symbol: str) -> go.Figure:
        ...
        # Contract prices
        fig.add_trace(
            go.Scatter(x=front["Datetime"], y=front["Close"], name=f"{front_symbol}"), row=1, col=1
        )

        fig.add_trace(
            go.Scatter(x=back["Datetime"], y=back["Close"], name=f"{back_symbol}"), row=1, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(x=spreads["Datetime"], y=spreads["spread"], name="Spread", fill="tozeroy"),
            row=2,
            col=1,
        )
